...
namespace pgsql
{
    public partial class GroupsWindow : Form
    {
        ...
        {
            dtData = sql.getListGroup(); // загрузка данных
        ...
        private void bEdit_Click(object sender, EventArgs e)
        {
            ...
            if (dtData != null && dgvData != null && dgvData.RowCount > 0 && dgvData.CurrentRow.Index != -1)
            {
                ...
                EditGroupWindow frm = new EditGroupWindow(dtRows.Rows[dgvData.CurrentRow.Index]);
                frm.ShowDialog();
        ...
        private void bDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                DataTable dtRows = dtData.DefaultView.ToTable();
             // проверка людей в группах
                DataTable dtR = sql.getGroupInPeople(Convert.ToInt32(dtRows.Rows[dgvData.CurrentRow.Index]["id"].ToString()));
                ...
                if (MessageBox.Show(message, "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    // удаляю группу из таблицы группы
                    sql.delGroup(int.Parse(dtRows.Rows[dgvData.CurrentRow.Index]["id"].ToString()));
        ...
        {
            // добавить
            AddGroupWindow frm = new AddGroupWindow();
            frm.ShowDialog();
